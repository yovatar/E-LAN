    ...
    <div
        ...
        <div class="flex flex-col-reverse justify-between px-6 py-3 space-y-2 lg:flex-row lg:space-y-0 lg:space-x-6">
            <div class="flex flex-col flex-grow space-y-3">
                ...
                <form action="/settings/account" method="POST" class="flex flex-col space-y-2">
                    ...
                    <div role="old password field" class="relative" x-data="{show : false}">
                        <input x-bind:type="show ? 'text' : 'password'" name=" oldPassword" id="oldPassword"
                        ...
                        <button type="button" tabindex="-1"
                            ...
                            <svg x-show="show" x-cloak xmlns="http://www.w3.org/2000/svg" class="w-5 h-5"
                    ...
                    <div role="new password field" class="relative" x-data="{show : false}">
                        <input x-bind:type="show ? 'text' : 'password'" name=" newPassword" id="newPassword"
                            class="w-full border-2 rounded-md border-blueGray-200 focus:outline-none focus:ring-2 focus:ring-purple-500"
                            required>
                        <button type="button" tabindex="-1"
                            class="absolute inset-y-0 right-0 flex items-center px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                    ...
                    <div role="new password confirm field" class="relative" x-data="{show : false}">
                        <input x-bind:type="show ? 'text' : 'password'" name=" newPasswordConfirm"
                    ...
                    <button type="submit"
                        class="px-6 py-3 font-medium text-white bg-purple-500 rounded-md hover:bg-purple-700 focus:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-300">
                        <p>Mettre à jour</p>
                    </button>
                </form>
            </div>
            <form action="/settings/account" method="POST" enctype="multipart/form-data"
                ...
                <button type="submit"
                    class="w-full px-6 py-3 font-medium text-white bg-purple-500 rounded-md hover:bg-purple-700 focus:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-300">Appliquer</button>
            </form>
        </div>
    </div>
</div>
