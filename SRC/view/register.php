...
<div class="flex flex-row justify-center w-full">
    <form action="/authentication/register" method="POST"
        ...
        <div role="username field" class="flex flex-col" x-data="{available : null}">
            <div class="relative w-full">
                <input x-ref="iptUsername" x-on:change="if($refs.iptUsername.value.length){
                    ...
                    x-bind:class="{ 'border-blueGray-200' : available === null, 'border-green-500': available === true, 'border-red-500' : available === false}"
                    type="text" id="username" name="username" placeholder="bobby" required
                    class="w-full border-2 rounded-md border-blueGray-200 focus:outline-none focus:ring-2 focus:ring-purple-500 h-11">
                <div x-show="available === false" x-cloak
                    class="absolute inset-y-0 right-0 flex items-center px-4 py-2 text-red-500 rounded-md">
                ...
                <div x-show="available === true" x-cloak
                    class="absolute inset-y-0 right-0 flex items-center px-4 py-2 text-green-500 rounded-md">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd"
        ...
        <div role="email field" class="flex flex-col" x-data="{available : null}">
            <div class="relative w-full">
                <input x-ref="iptEmail" x-on:change="if($refs.iptEmail.value.length){
                    ...
                    x-bind:class="{ 'border-blueGray-200' : available === null, 'border-green-500': available === true, 'border-red-500' : available === false}"
                    type="email" id="email" name="email" placeholder="bob.ross@cpnv.ch" required
                    class="w-full border-2 rounded-md border-blueGray-200 focus:outline-none focus:ring-2 h-11 focus:ring-purple-500">
                <div x-show="available === false" x-cloak
                    class="absolute inset-y-0 right-0 flex items-center px-4 py-2 text-red-500 rounded-md">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd"
        ...
        <input type="text" id="lastName" name="lastName" placeholder="Ross" required
            class="border-2 rounded-md h-11 border-blueGray-200 focus:outline-none focus:ring-2 focus:ring-purple-500">
        ...
        <div role="passwordField" class="relative" x-data="{show : false}">
            <input
                :class="{'border-blueGray-200':$store.passwordCheck.valid === null,'border-red-500':$store.passwordCheck.valid === false, 'border-green-500': $store.passwordCheck.valid === true}"
                x-bind:type="show ? 'text' : 'password'" id="password" name="password" required
                @change="$store.passwordCheck._password = $event.target.value"
                class="w-full border-2 rounded-md border-blueGray-200 h-11 focus:outline-none focus:ring-2 focus:ring-purple-500">
        ...
        <div role="passwordField" class="relative" x-data="{show : false}">
            <input
                :class="{'border-blueGray-200':$store.passwordCheck.valid === null,'border-red-500':$store.passwordCheck.valid === false, 'border-green-500': $store.passwordCheck.valid === true}"
                x-bind:type="show ? 'text' : 'password'" id="passwordCheck" name="passwordCheck" required
                @change="$store.passwordCheck._confirm = $event.target.value"
                class="w-full border-2 rounded-md h-11 border-blueGray-200 focus:outline-none focus:ring-2 focus:ring-purple-500">
            <button type="button" tabindex="-1"
                class="absolute inset-y-0 right-0 flex items-center px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                @click="show = !show">
                <svg x-show="!show" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20"
        ...
        <div class="flex flex-col space-y-2 lg:flex-row lg:items-center lg:space-x-3 lg:space-y-0">
            ...
            <p>vous avez deja un compte? <a href="/authentication/login"
                    class="text-purple-500 hover:text-purple-700">connectez vous</a></p>
        </div>
    </form>
</div>
<?php
