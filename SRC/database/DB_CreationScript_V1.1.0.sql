...
CREATE TABLE IF NOT EXISTS `elan`.`users` (
  ...
  UNIQUE INDEX `UniqueUserUsername` (`username` ASC) VISIBLE,
  INDEX `fk_users_images1_idx` (`image_id` ASC) VISIBLE,
  UNIQUE INDEX `UniqueUserEmail` (`email` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`states` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueState` (`type` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `elan`.`lans` (
  ...
  `images_id` INT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueLan` (`name` ASC) VISIBLE,
  INDEX `fk_Lans_Stats1_idx` (`state_id` ASC) VISIBLE,
  INDEX `fk_lans_images1_idx` (`images_id` ASC) VISIBLE,
  CONSTRAINT `fk_Lans_Stats1`
    FOREIGN KEY (`state_id`)
    REFERENCES `elan`.`states` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_lans_images1`
...
CREATE TABLE IF NOT EXISTS `elan`.`locations` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueLocation` (`name` ASC, `address` ASC) INVISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `elan`.`events` (
  ...
  `image_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_events_images1_idx` (`image_id` ASC) VISIBLE,
  UNIQUE INDEX `UnqiqueEvent` (`name` ASC, `start` ASC, `end` ASC) VISIBLE,
  CONSTRAINT `fk_events_images1`
    FOREIGN KEY (`image_id`)
    REFERENCES `elan`.`images` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `elan`.`teams` (
  ...
  INDEX `fk_Teams_Users1_idx` (`owner_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`tournaments` (
  ...
  `name` VARCHAR(64) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueTournament` (`name` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `elan`.`matches` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_Matches_Teams_idx` (`team1_id` ASC) VISIBLE,
  INDEX `fk_Matches_Teams1_idx` (`team2_id` ASC) INVISIBLE,
  INDEX `fk_Matches_Tournaments1_idx` (`tournament_id` ASC) VISIBLE,
  UNIQUE INDEX `UniqueMatch` (`match_number` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`tournament_is_event` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueTournamentEvent` (`tournament_id` ASC, `event_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`lan_contains_event` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueLanEvent` (`lan_id` ASC, `event_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`team_participate_in_tournament` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueParticipation` (`team_id` ASC, `tournament_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`event_is_at_location` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueEventLocation` (`event_id` ASC, `location_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`user_register_to_event` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UnqiqueUserEvent` (`event_id` ASC, `user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `elan`.`user_joins_team` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `UniqueUserTeam` (`user_id` ASC, `team_id` ASC) VISIBLE,
